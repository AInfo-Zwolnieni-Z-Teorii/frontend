  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 pt-24 bg-slate-50">
      ...
      <div className="w-full bg-white p-6 pb-1 rounded-lg shadow-lg flex flex-col items-center justify-center">
        <h1 className="pt-10 pb-1 mb-9 text-3xl font-light text-blue-600 border-b-2 border-blue-600">{name}</h1>
        <div className="grid grid-cols-4 gap-4 pt-10 pb-4 w-3/4 mx-auto">
        {categories.map((type) => (
          <div
            key={type}
            className={"lg:px-12 md:px-8 lg:py-8 md:py-5 py-3 px-5 rounded-md font-medium text-sm lg:text-lg xl:text-xl shadow-lg bg-gray-50 text-gray-800 flex align-center justify-center"
            }
          >
            {type}
          </div>
      ...
      <div className="py-10 w-3/4 mx-auto">
        <img src={URL.createObjectURL(image)} alt="wybrane zdjecie" className="w-full"/>
      </div>
